...
const createReport = (test, index) => {
  ...
  return {
    ...
    meta: {
      learning_goals: [...test.ancestorTitles],
    },
  };
};
...
class MyCustomReporter {
  ...
  async onRunComplete(contexts, suiteResults) {
    ...
    let score = 0;
    const testReports = suiteResults.testResults
      .flat()
      .map((fileResult) => {
    ...
    const dataToFile = JSON.stringify(output);

    // Check for connection of the user to internet
    require("dns").resolve("www.google.com", function (err) {
      if (err) {
        // console.log("No connection");
        //if no connection we save the result in a file.
        fs.writeFileSync("output.txt", dataToFile, (err) => {
    ...
    axios
      .post(`${baseUrl}/raw_data`, output)
      .then(function (response) {
        // console.log("from axios calll!!!!", response.data.message);
      })
    ...
    const testOutput = {
      ...
      evaluation: [
        ...
        },
        {
          ...
          attempted: true,
          passed: true,
          key: "[C] calculateTotalEggDistance: calculates how for you have to walk to hatch one of each pokemon egg",
          failureMessages: [],
          meta: [Object],
        },
        {
          ...
          attempted: true,
          passed: false,
          key: "[D] getHeaviestPokemon: returns the heaviest pokemon from an array of pokemons",
          failureMessages: [],
          meta: [Object],
        },
        {
    ...
    axios
      .post(`${baseUrl}/evaluations`, output)
      .then(function (response) {
